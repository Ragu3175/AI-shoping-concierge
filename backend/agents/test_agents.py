import json
from agents.crew_setup import run_agent_pipeline


def print_product(product: dict, label: str):
    if not product:
        print(f"{label}: None")
        return
    ...
    print(f"  Badge: {product.get('badge')} | Price: {product.get('price')}")
...
def main():
    ...
    print_product(top_pick, "TOP PICK")

    alternatives = result.get("alternatives", [])
    print(f"ALTERNATIVES ({len(alternatives)}):")
    for idx, alt in enumerate(alternatives, 1):
        print_product(alt, f"ALTERNATIVE {idx}")
